...
| 27 | Two kinds of shortcut, edited in different places | A **global** shortcut is a role — key 2 means "the modelling pie here", and which pie that is depends on the workbench, so several pies compete for it and the key itself belongs in Preferences. A **direct** shortcut is an address — key 9 means one named pie, anywhere, so it belongs on the pie. Editing them in the same control makes them read as variants of one setting. Direct keys must also be rejected if they collide with a global key or another pie's direct key. |
